...
def fixup(atoms, mol, indicators):
    ...
    mol.SetAromaticPerceived(True)  # avoid perception 感知芳香性？
    # print(len(atoms))
    # print(indicators)
...
def reconstruct_from_generated(pos, atom_type, indicators):
    # xyz = data.ligand_context_pos.clone().cpu().tolist()
    # atomic_nums = data.ligand_context_element.clone().cpu().tolist()
    # indicators = data.ligand_context_feature_full[:, -len(ATOM_FAMILIES_ID):].clone().cpu().bool().tolist()
    xyz = pos.clone().cpu().tolist()
    atomic_nums = atom_type.clone().cpu().tolist()
    indicators = indicators.clone().cpu().bool().tolist()

    mol, atoms = make_obmol(xyz, atomic_nums)
    fixup(atoms, mol, indicators)

    connect_the_dots(mol, atoms, indicators, 4)
    fixup(atoms, mol, indicators)
    mol.EndModify()
...
def reconstruct_from_generated_dpm(data):
    xyz = data.ligand_context_pos.clone().cpu().tolist()
    atomic_nums = data.ligand_element.clone().cpu().tolist()
    indicators = data.ligand_context_feature_full.clone().cpu().bool().tolist()
...
def reconstruct_from_generated_data(atomic_nums, xyz):
    # xyz = data.ligand_context_pos.clone().cpu().tolist()
    # atomic_nums = data.ligand_element.clone().cpu().tolist()
    # indicators = data.ligand_context_feature_full.clone().cpu().bool().tolist()
    xyz = xyz.tolist()
    ...
    mol, atoms = make_obmol(xyz, atomic_nums)
    fixup(atoms, mol, None)

    connect_the_dots(mol, atoms, None, 2)
    fixup(atoms, mol, None)
    mol.EndModify()
...
def reconstruct_from_generated_with_edges(xyz, atomic_nums, bond_index, bond_type, raise_error=True, sanitize=True):
    ...
    atomic_nums = atomic_nums.clone().cpu().tolist()
    xyz = xyz.clone().cpu().tolist()
    bond_type = bond_type.clone().cpu().tolist()
    bond_index = bond_index.clone().cpu().tolist()
